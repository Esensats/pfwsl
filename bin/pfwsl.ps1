...
elseif ($env:WslIp) {
  ...
  if ($c -ne 'iprm') {
    ...
    Write-Host "You can remove it with the iprm command.";
  }
}
else {
  try { 
    ...
    throw "Failed to get the ip address of WSL 2: $_"
  }
...
switch ($c) {
  ...
  'ipset' {
    ...
    Write-Output "Set WSL IP to `$env:WslIp = $remoteport"
    return
  }
  'iprm' {
    if (-not $env:WslIp) {
      Write-Output "WSL IP is not set in `$env:WslIp"
      return
    }
    Remove-Item Env:\WslIp
    Write-Output "Removed WSL IP from `$env:WslIp"
    return
  }
  default {
